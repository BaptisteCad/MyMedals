...
export class DiagramPage {
  ...
  public myDiagram = undefined;

  private array = [
    { key: 0, n: "Father", s: "M", m:7, f:8, ux: 1,a: ["C", "F", "K"] },
    { key: 1, n: "Mother", s: "F", m:9, f:10, vir: [0, 5], a: ["B", "H", "K"] },
    { key: 5, n: "Mother's Spouse", s: "M", a: ["C"] },
    { key: 6, n: "Maternal Halfbrother", s: "M", m: 1, f: 5, a: ["C"] },
    { key: 4, n: "Claire", s: "F", m: 1, f: 0, vir: 3, a: ["C"] },
    { key: 3, n: "Claire's husband", s: "F", m: 0, f: 0, ux: 4,a: ["C"] },
    { key: 12, n: "Baby", s: "F", m: 4, f: 3, a: ["C"] },
    { key: 13, n: "Baby2", s: "F", m: 4, f: 3, a: ["C"] },
    // ancestors
  ...
  init(array) {
    ...
    this.setupDiagram(this.myDiagram, array,
  ...
  setupMarriages(diagram) {
    ...
    for (var i = 0; i < nodeDataArray.length; i++) {
      ...
      if (virs !== undefined) {
        ...
        for (var K = 0; K < virs.length; K++) {
          var husband = virs[K];
          if (key === husband) {
            // or warn no reflexive marriages
            continue;
          }
          var link2 = this.findMarriage(diagram, key, husband);
          if (link2 === null) {
            // add a label node for the marriage link2
            var mlab2 = { s: "LinkLabel", key: 0 };
            model.addNodeData(mlab2);
            // add the marriage link itself, also referring to the label node
            var mdata2 = { from: key, to: husband, labelKeys: [mlab2.key], category: "Marriage" };
            model.addLinkData(mdata2);
          }
        }
      }
    }
  ...
  add(net, coll, nonmemberonly) {
    ...
    while (it.next()) {
      ...
      if (node.isLinkLabel) {
        ...
        node.linksConnected.each(function(l) { if (l.isLabeledLink) marriagesCount++; });
        if (marriagesCount === 0) {
          vertex = net.addNode(node);
    ...
    while (multiSpousePeople.count > 0) {
      ...
      node = multiSpousePeople.first();
      var cohort = new go.Set<go.Node>();
      this.extendCohort(cohort, node);
      // then encourage them all to be the same generation by connecting them all with a common vertex
      var dummyvert = net.createVertex();
      net.addVertex(dummyvert);
      var marriages = new go.Set<go.Link>();
  ...
  commitNodes() {
    ...
    this.network.vertexes.each(function(v) {
      ...
      if (spouseA.opacity === 0) {
        ...
        var pos2 = new go.Point(v.centerX - spouseB.actualBounds.width / 2, v.y);
        spouseA.position = pos2;
        spouseB.position = pos2;
      }
    });
